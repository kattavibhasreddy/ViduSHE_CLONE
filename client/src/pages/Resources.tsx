...
const Resources = () => {
  ...
  return (
    <section id="resources" className="py-12 md:py-20 bg-white">
      <div className="container mx-auto px-4">
        <div className="text-center mb-16">
          <h1 className="text-2xl md:text-3xl font-bold text-primary mb-4">Resource Library</h1>
          <p className="text-lg text-neutral-700 max-w-2xl mx-auto">Access our collection of voice-searchable resources to support your journey.</p>
        </div>
        ...
        <div className="max-w-2xl mx-auto mb-12">
          <div className="relative">
            <Input
              ...
              className="w-full px-4 py-3 pl-12 border border-gray-200 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent"
            />
            ...
            <Button 
              ...
              className={`absolute inset-y-0 right-0 flex items-center px-4 text-primary hover:text-primary-dark transition-all ${isListening ? 'animate-pulse' : ''}`}
            >
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredResources.map((resource) => (
            <Card key={resource.id} className="bg-gray-50 hover:shadow-lg transition-all">
              <CardContent className="p-6">
                <div className="flex justify-between items-start mb-4">
                  <Badge variant="outline" className="bg-primary-100 text-primary text-xs font-medium px-2.5 py-1 rounded-full capitalize">
                    {resource.category}
                  </Badge>
                  <Button
                    variant="ghost"
                ...
                <h3 className="text-lg font-semibold mb-2 text-neutral-900">{resource.title}</h3>
                <p className="text-neutral-700 text-sm mb-4">{resource.description}</p>
                <Button 
                  variant="link" 
                  className="text-[#DD6B20] hover:text-[#BA5A1A] font-medium text-sm flex items-center p-0"
                >
                  Access Resource
